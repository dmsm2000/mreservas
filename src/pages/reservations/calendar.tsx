...
import { Calendar, CalendarProps, EventPropGetter, momentLocalizer } from "react-big-calendar";
...
export default function MyCalendar() {
  ...
  /* const [momentInstance, setMomentInstance] = useState<Moment>(moment());
  const [localizer, setLocalizer] = useState(momentLocalizer(momentInstance)); */

  /* useEffect(() => {
    const newMomentInstance = moment();
    newMomentInstance.locale(i18n.language);
    setMomentInstance(newMomentInstance);
  }, [i18n.language]); */
  

 /*  useEffect(() => {
    setLocalizer(momentLocalizer(momentInstance));
  }, [momentInstance]); */
  ...
  const handleDelete = () => {
    setIsLoading(true)
    deleteDocumentWithId(Collections.reservations, selectedReservation!.getId()!).then(() => {
      setIsLoading(false)
    }).catch((error) => {
      setIsLoading(false)
    })
  ...
  const handleSelectSlot = (slotInfo: { start: Date; end: Date }) => {
    setSelectedReservation(new Reservation(null, "", slotInfo.start, slotInfo.end, ReservationStates.booked.value, "", "", "", undefined));
    setShowReservationDialog(true)
  };
  ...
  const reservationDialog = () => {
    return (<ReservationDialog handleClose={(wasSuccess: any, message: string) => {
      setShowReservationDialog(false)
      if (wasSuccess === true) {
    ...
    }} handleDelete={handleDelete} reservation={selectedReservation} handleAddReservation={() => { }} />)
  }
  ...
  const calendarProps: CalendarProps<any, any> = {
    ...
    onSelectEvent: (selectedReservation: Reservation) => { handleSelectEvent(selectedReservation) },
    onSelectSlot: (slotInfo: { start: any; end: any }) => handleSelectSlot(slotInfo),
  ...
  const messages = {
    ...
    showMore: (count: number) => t('getCalendar.getShowMoreButonLabel', { count }),
    };
  ...
  return (
    <>
      ...
      <Button style={{ marginBottom: "20px" }} variant="contained" startIcon={<AddIcon />}
        onClick={addReservation}> {t('getCreateEvent')} </Button>
      </div>
      <Calendar messages={messages} style={{ height: "80vh" }} {...calendarProps} eventPropGetter={(reservation) => {
