...
export default function ReservationDialog(props: ChildProps) {
  ...
  const [selectedService, setSelectedService] = useState<Service | undefined>(
    props.reservation != null ? props.reservation.service : undefined
  );

  const [name, setName] = useState(
    props.reservation != null ? props.reservation.name : ""
  );
  const [phone, setPhone] = useState(
    props.reservation != null ? props.reservation.phone : ""
  );
  const [email, setEmail] = useState(
    props.reservation != null ? props.reservation.email : ""
  );
  const [observations, setObservations] = useState(
    props.reservation != null ? props.reservation.observations : ""
  );
  const [startDate, setStartDate] = useState<Date>(
    props.reservation != null ? props.reservation.start : new Date()
  );
  const [endDate, setEndDate] = useState<Date>(
    props.reservation != null ? props.reservation.end : new Date()
  );
  const [state, setState] = useState(
    props.reservation != null ? props.reservation.state : ""
  );

  const [showDialogConfirmDelete, setShowDialogConfirmDelete] = useState(false);
  const [showDialogWrongDate, setShowDialogCWrongDate] = useState(false);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      getCollectionRef(Collections.services),
      (querySnapshot) => {
  ...
  function verifyReservationData(): boolean {
    if (
    ...
    <>
      ...
      <Dialog open={true} onClose={() => props.handleClose(null, "")} fullWidth>
        ...
        <DialogContent>
                ...
                onChange={(e) => {
                  ...
                  if (selectedService) {
                    ...
                    const newEndDate = new Date(
                      newStartDate.getTime() + duration * 60000
                    );
                ...
                <FormControl fullWidth margin="dense">
                  ...
                  >
                    {Object.keys(ReservationStates).map((state) => (
                      <MenuItem
                        key={ReservationStates[state].value}
                        value={ReservationStates[state].value}
                      >
          ...
          <DialogContentText
            style={{ color: "red", marginTop: "5px", fontSize: "smaller" }}
          >
        ...
        <DialogActions>
          ...
          {reservation?.getId() != null && (
            ...
            >
              {t("getDeleteButtonLabel")}
            </Button>
          )}
          <Button
            onClick={() => {
              verifyReservationData() ? setIsLoading(true) : setIsLoading(false);
            }}
          >
            {reservation?.getId() != null
              ? t("getUpdateButtonLabel")
              : t("getCreateButtonLabel")}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* Wrong Data Dialog */}
      <Dialog
        open={showDialogWrongDate}
        keepMounted
        onClose={() => setShowDialogCWrongDate(false)}
        aria-describedby="alert-dialog-slide-description"
      >
        <DialogTitle> {t("getImpossibilityReservationTitle")} </DialogTitle>
        <DialogContent>
          <DialogContentText>
            {t("getImpossibilityReservationMessage")}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
            style={{ color: "red" }}
            onClick={() => setShowDialogCWrongDate(false)}
          >
            {" "}
           {t("getOkButtonLabel")}{" "}
          </Button>
